...
# the same commit. `content/index.md` is regenerated by every sync, so its mtime
# moves and Quartz — finding no date in its frontmatter — stamps the build time
# into `<time datetime="…">`. Hashing that raw made the homepage never match.
fingerprint() { sed 's/datetime="[^"]*"/datetime=""/g' | md5sum | cut -d' ' -f1; }
