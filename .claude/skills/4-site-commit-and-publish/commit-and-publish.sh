...
# Checksum of a page, ignoring what legitimately differs between two builds of
# the same commit — all of it measured against the real deploys, not assumed:
#
#   * `content/index.md` is regenerated by every sync, so its mtime moves and
#     Quartz — finding no date in its frontmatter — stamps the build time into
#     `datetime`;
#   * the CI runs in UTC and this machine does not, so the *rendered* date can
#     land on another day (22:05Z showed as "09 août" there, "10 août" here).
#
# Hence the whole <time> element goes, attribute and text.
fingerprint() { sed 's|<time[^>]*>[^<]*</time>|<time/>|g' | md5sum | cut -d' ' -f1; }
...
if [ "$PUSH" -eq 1 ]; then
  ...
  # Which pages to verify. Quartz output is reproducible once the volatile bits
  # are stripped (see fingerprint), so comparing a local build to the served page
  # proves the *new* content is live. Merely checking that the site answers, or
  # that two reads agree, passes happily on a stale CDN copy.
  #
