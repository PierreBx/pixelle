...
if [ "$PUSH" -eq 1 ]; then
  ...
  # Which pages to verify: the pages this push actually changed. Quartz output is
  # reproducible once the volatile bits are stripped (see fingerprint), so
  # comparing a local build to the served page proves the *new* content is live.
  # Merely checking that the site answers, or that two reads agree, passes happily
  # on a stale CDN copy.
  #
  # The slug comes from Quartz's own slugifyFilePath, never from a guess: naive
  # space-to-dash gave `Jardins-de-Marqueyssac.html`, which Quartz emits as a
  # 301-byte `<meta refresh>` stub beside the real page. That stub never changes,
  # so the check passed on every deploy without ever looking at real content.
  #
  # The generated homepage is deliberately absent: having no date in its
  # frontmatter, Quartz stamps the build time into it, so it never matches.
  mapfile -t targets < <(
    git diff --name-only "$base" HEAD -- content 2>/dev/null \
      | grep '\.md$' | grep -v '^content/index\.md$' | head -3 \
      | node --input-type=module -e '
import { readFileSync } from "node:fs"
import { slugifyFilePath } from "@quartz-community/utils"
for (const line of readFileSync(0, "utf8").split("\n").filter(Boolean))
  console.log(slugifyFilePath(line.replace(/^content\//, "")) + ".html")
' 2>/dev/null
  )

  if [ "${#targets[@]}" -eq 0 ]; then
    warn "deploy" "no content page changed — nothing to compare, deploy not verified"
    step "deploy" "pushed; check by hand if it matters — $SITE"
    exit 0
  fi
  ...
  for i in $(seq 1 40); do
    matched=1
    compared=0   # sans ce compte, tout ignorer vaudrait « tout concorde »
    for t in "${targets[@]}"; do
      [ -f "public/$t" ] || continue
      comparable "public/$t" || continue
      compared=$((compared + 1))
      local_sum="$(fingerprint < "public/$t")"
      served="$(curl -sf -H 'Cache-Control: no-cache' "$(page_url "$t")?cachebust=$i" 2>/dev/null | fingerprint)"
      [ "$served" = "$local_sum" ] || { matched=0; break; }
    done
    if [ "$compared" -eq 0 ]; then
      warn "deploy" "changed pages are all bases or missing from public/ — not verifiable"
      step "deploy" "pushed; check by hand if it matters — $SITE"
      exit 0
    fi
    if [ "$matched" -eq 1 ]; then
      step "deploy" "$compared served page(s) match the local build — $SITE"
      exit 0
    fi
    sleep 15
  done
